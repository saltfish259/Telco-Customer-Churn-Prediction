...
if pipeline is None:
    st.error(
        ...
        "Please place `lightgbm_pipeline.pkl` inside the `models/` folder."
    )
    st.stop()
...
st.info(f"💡 Estimated Total Charges: **${total_charges:,.2f}**  *(calculated from tenure × monthly charges)*")
...
if predict_clicked:
    try:
        # Build input dataframe with the exact feature names the pipeline expects
        ...
        st.error(f"❌ Prediction failed. Please check your inputs and try again.")
